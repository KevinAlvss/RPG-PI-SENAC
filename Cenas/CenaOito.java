...
public class CenaOito {
    ...
    private static void irFalarComEles() {
        Utils.escrever(
                "Opa casal , tudo certo? Alan diz ``tudo sim´´, e Joan diz ``tudo certo, e você?´´, eu digo ``Estou com uma pulga atrás da orelha, se a Helen não conhece o rapaz alemão como ela sabe que ele namora´´");
        Utils.escrever(
    ...
    private static void comentarSobre() {
        Utils.escrever(
                "intrigado com que eu acabei de ouvir de Helen eu digo´´ Vocês não acham engraçado.... como que ela sabe que o rapaz alemão tem uma namorada?´´");
        Utils.escrever(
    ...
    private static void chegarNoEscritorio() {
        Utils.escrever(
                "Ao chegarmos todos lá Alan diz ``E se o Christopher não precisar testar todas as combinações \ne sim por padrões especificos... Hugh diz ``por palavras especificas´´");
